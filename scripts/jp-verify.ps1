...
try {
  ...
  if (-not (Get-Module -ListAvailable -Name PSScriptAnalyzer)) {
    ...
    if ($errors.Count -gt 0) {
      $errors | ForEach-Object { Write-Host ("PSSA: " + [CmdletBinding()]
param(
  [switch]$Quiet,
  [switch]$NoStop
)
...
try {
  ...
  Say ("PASTE NEEDED (verify fail): " + [CmdletBinding()]
param(
  [switch]$Quiet,
  [switch]$NoStop
)
...
.Exception.Message)throw   # IMPORTANT: do not swallow failures
}
finally {
  if (-not $NoStop) {
    if ($didFail) {
...
.RuleName + " — " + $_.Message + " (" + $_.ScriptName + ":" + $_.Line + ")") }
      throw "PSScriptAnalyzer errors: $($errors.Count)."
    }
    Say "PSScriptAnalyzer OK."
  }
